...
class PongConsumer(AsyncWebsocketConsumer):
	...
	async def connect(self):
		...
		if user.is_authenticated:
			if user.username in self.disconnected_players:  # handle user reconnection
				old_session_id = self.disconnected_players[user.username]['session_id']
				if old_session_id == session_id and time.time() < self.disconnected_players[user.username]['rejoin_deadline']:
			...
			if session_id in self.game_sessions and len(self.game_sessions[session_id]['players']) < 2:  # handle new user connection
				...
				print("No available game session. Creating a new game session...")
				await self.close()
		else:
	...
	async def disconnect(self, close_code):
		if hasattr(self, 'session_id') and self.session_id in self.game_sessions:
			...
			if player_name:
				...
				if not self.game_sessions[self.session_id]['players']:
					del self.game_sessions[self.session_id]	
	...
	async def send_game_result(self, result, session_id, winner):
		...
		await self.update_tournament(session_id, winner)
	...
	@sync_to_async
	def update_tournament(self, session_id, winner):
		tournament = cache.get('tournament')
		if tournament is None:
			return

		# Find the match with the corresponding session_id
		for round_type in ['semi_finals', 'final']:
			if round_type == 'semi_finals':
				for match in tournament[round_type]:
					if match['session_id'] == session_id:
						# Update the winner in the final
						tournament['final']['player1' if tournament['final']['player1'] is None else 'player2'] = winner
						break
			elif round_type == 'final':
				if tournament[round_type]['session_id'] == session_id:
					# Tournament is over, we can add additional logic here if needed
					pass
