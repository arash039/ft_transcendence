	...
	async def disconnect(self, close_code):
		if hasattr(self, 'session_id') and self.session_id in self.game_sessions:
			...
			if len(self.game_sessions[self.session_id]['players']) == 0:
				...
				if player_name == list(self.game_sessions[self.session_id]['players'].values())[0]:
					winner = list(self.game_sessions[self.session_id]['players'].values())[1]
				else:
					winner = list(self.game_sessions[self.session_id]['players'].values())[0]
				result = {
					'players': list(self.game_sessions[self.session_id]['players'].values()),
					'winner': winner,
					'score': 'disconnected opponent'
				}
				...
				asyncio.create_task(self.send_game_result(result, self.session_id))
				self.reset_game(self.session_id)
